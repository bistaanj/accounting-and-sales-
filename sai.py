...
class Window(Tk):
    ...
    def navigationFrame_inventory(self, tab):
        ...
        self.btn_addProduct = ttk.Button(
            self.buttonFrame, text="Add New Product", style='TButton', command=self.addNewRecord)
        self.btn_addProduct.grid(column=0, row=1, pady=10)

        self.btn_update = ttk.Button(
            self.buttonFrame, text="Update Inventory", style='TButton', command=self.updateInventory)
        ...
        backupBtn = ttk.Button(
            self.buttonFrame, text="Day End", style='TButton', command=self.dayEnd)
    ...
    def backupAndRecovery(self):
        ...
        def backupDatabase():
            ...
            validate = messagebox.askyesno(
                'Backup Request', "Do you want to initiate backup process?")
            if (validate):
                top = Toplevel()
                top.grab_set()
                lbl = Label(
                    top, text=" Backing up Database. Do not close the program ")
                lbl.pack()
                top.geometry("+%d+%d" % (400, 300))

                # Backs up in D drive by default
                command = 'mongodump --db saiRecords --host localhost:27017 --out E:/databaseBackupSI/' + \
                    str(date)+'_'+str(time)
                subprocess.call(command, shell=True)
                # os.system(command)
                top.destroy()
                messagebox.showinfo("Request Completed",
        ...
        backup_btn = ttk.Button(
            self.displayFrame, text='Back-up Database', command=backupDatabase)
        backup_btn.pack(padx=50, pady=20, anchor='e')
        tips = Label(self.displayFrame,
                     text='(Use this option to back-up database)')
        tips.pack(padx=20)

        backup_btn = ttk.Button(
            self.displayFrame, text='Recover Database', command=restoreDatabase)
        backup_btn.pack(padx=50, pady=20, anchor='e')
        tips = Label(self.displayFrame,
    ...
    def dayEnd(self):
        try:
            ...
            validate = bool(0)
            validate = messagebox.askyesno(
                'Conformation Required', 'Do you want to send the data?')
            if(validate):
    ...
    def addNewRecord(self):
        ...
        s = ttk.Style()
        s.configure('TLabel', font=('Helvetica', 18, 'bold'),
        ...
        productNameEntry = Entry(
            self.displayLabel, width=50, border=0, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        PtypeCombo = ttk.Combobox(self.displayLabel, background='#CED7D7', values=[
                                  'Pcs', 'Pkts', 'Liters', 'Bundle', 'Kgs', 'Meter', 'Other'], font=('Comic Sans MS', 10, 'bold'))
        ...
        productCostEntry = Entry(
            self.displayLabel, width=20, border=0, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        locationEntry = Entry(self.displayLabel, width=20,
                              border=0, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        productDescription = Entry(
            self.displayLabel, border=0, width=50, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        self.submit_record_btn.grid(
            column=0, row=8, padx=10, pady=20, sticky="se")

        tips = Label(self.displayLabel, text="*record validations:-",
                     font=('Times New Roman', 10, 'underline'), fg='red', bg='white')
    ...
    def updateInventory(self):
        ...
        def displayUpdatePopup(displayText, cmd):
            ...
            updateBtn = Button(
                self.UpdatePopUp, text="Update Value", command=cmd)
        ...
        def dbsQuantityUpdate(event=''):
            try:
                ...
                databaseRow = collection.find_one(
                    {'_id': ObjectId(iid)}, {'Quantity': 1, '_id': 0})
                # connection.close()
        ...
        def updateLocationDbs():
            ...
            if self.row_iid == 0:
                ...
                displayUpdatePopup("Update Value", dbsLocationUpdate)
        ...
        self.searchEntry = Entry(self.searchFrame, width=40, bg='#4F83FC',
                                 fg='#FFFFFF', border=0, font=('Comic Sans MS', 15))
        ...
        lb = Label(self.rsltFrame, text='Search Result', font=(
            'Helvetica', 15, 'bold', 'underline'), bg='white')
        lb.pack(pady=20)
        # Table view starts from here
        viewTree = ttk.Treeview(self.rsltFrame, height=8,
                                style="mystyle.Treeview")

        # Define Columns
        viewTree['columns'] = ('Product Name', 'Cost Price',
                               'Sales Price', 'Quantity', 'Units', 'Location')
        ...
        changeCost = Button(
            self.btnFrame, text="Update Cost", command=updateCostDbs)
    ...
    def viewInventory(self):
        ...
        searchBtn = Button(searchFrame, text="GO", command=displaySearchResult,font=('Times New Roman', 18, 'bold', 'underline'), bg='#4F83FC', fg='#FFFFFF', border=0, cursor="hand2")
        searchBtn.grid(column=2, row=1, padx=10, pady=10, sticky="w")

        viewTree = ttk.Treeview(
            self.displayFrame,  style="mystyle.Treeview", height=5)

        # Define Columns
        viewTree['columns'] = ('Product Name', 'Cost Price','Sales Price', 'Quantity', 'Units', 'Location', 'Purchased From')
        viewTree.column('#0', width=60, minwidth=10, anchor=CENTER)
        ...
        for x in inventorydata:
            ...
            if (qnty == 0):
                qnty = "Out of Stock"
            viewTree.insert(parent='', index=END, iid=(x["_id"]), text=(self.txt+1), values=(
                x['Product Name'], x['Cost Price'], x['Sales Price'], qnty, x['Units'], x['Location'], x['Purchased From']))
            self.txt += 1
        self.btnFrame = Frame(self.displayFrame, bg='pink')
        self.btnFrame.pack()
        connection.close()
    ...
    def navigationFrame_billing(self, tab):
        ...
        def vat_billing():
            if self.billing_method == 1:
                ...
                if (self.billingTotalAmount != 0):
                    validate = messagebox.askokcancel(
                        "Billing on Process", "Do you want to swiitch to VAT Billing ? ")
                    if (validate):
                        self.productsInBill = {}
                        self.billingTotalAmount = 0
                        self.billingAmountLabel.config(
                        ...
                        self.billingVatableAmountLabel.grid(
                            row=1, column=1, sticky="n",  pady=0)
                        self.VatableAmountLabel.grid(
        ...
        self.btn_addProduct = ttk.Button(
            self.buttonFrame, text="VAT Billing", style='TButton', command=vat_billing)
        self.btn_addProduct.grid(column=0, row=1, pady=10)

        self.btn_update = ttk.Button(
            self.buttonFrame, text="Order", style='TButton', command=order)
        self.btn_update.grid(column=0, row=2, pady=5)
        ...
        self.billingButtonFrame.pack(
            side='left', fill='both', pady=15, ipady=10)
        ...
        def completeBilling():
            ...
            def saveBilltoDbs(event=''):
                try:
                    ...
                    viewProductsInBill()
                    messagebox.showinfo(
                        'Transaction Completed', 'Bill saved to Database')
                except ValueError:
                    messagebox.showerror(
                        "Insuccifient Data", "Provide Customer Name")
            ...
            if (len(self.productsInBill) < 1):
                ...
                if(proceedBilling == 1):
                    ...
                    askEntry = Entry(top, width=30, font=(
                    ...
                    phnNumEntry = Entry(top, width=30, font=(
                        'Comic Sans MS', 15, 'bold'))
                    phnNumEntry.grid(row=1, column=1, padx=5, pady=5)
                    phnNumEntry.bind('<KeyRelease>', validateContact)
                    btn = Button(top, text="Enter", width=10,
                                 command=saveBilltoDbs)
       ...
       # New Add Product Funtion to add products in bill
            ...
            try:
                def displayToBillView(event=''):
                    ...
                    if (float(productToBill['Quantity']) < requiredQuantity or requiredQuantity < 1):
                        self.warnUser(
                            "Invalid Entry. Please Check the Available Quantity")
                        billingProcess()
                    else:
                        # try:
                        if (productToBill['Product Name'] in self.productsInBill.keys()):
                            ...
                            productTotal = int(
                                productToBill['Sales Price'])*requiredQuantity
                            ...
                            self.productsInBill[productToBill['Product Name']
                                                ]['Sales Price'] = productToBill['Sales Price']
                            self.productsInBill[productToBill['Product Name']
                ...
                displayAvailableQuantity = Label(top, text=(str(productToBill['Quantity'])+" " + str(
                    productToBill['Units'])), padx=5, pady=5, font=('Comic Sans MS', 15, 'bold'))
                displayAvailableQuantity.grid(row=0, column=1)

                salesPriceLbl = Label(
                    top, text="Sales Price", padx=5, pady=5, font=('Helvetica', 15, 'bold'))
                ...
                askQuantityLabel = Label(
                    top, text="Enter Quantity", padx=5, pady=5, font=('Helvetica', 15, 'bold'))
                askQuantityLabel.grid(row=2, column=0)

                askQuantityEntry = Entry(
                    top, width=10, font=('Comic Sans MS', 15, 'bold'))
                askQuantityEntry.grid(row=2, column=1)
                askQuantityEntry.focus()
                askQuantityEntry.bind('<Return>', displayToBillView)

                okBtn = Button(top, text="Sell", padx=5, pady=10, width=8, font=(
                    'Georgia', 10, 'bold'), command=displayToBillView)
        ...
        def billingEditProcess():
            def applyEdits(event=''):
                ...
                try:
                    if (newValue <= 0.0 or newValue > avalQuantity):
                        ...
                        billAmount = float(
                            (viewTree.item(iidEdit, 'values'))[4])
            ...
            if (iidEdit == ""):
                ...
                quantityLabel = Label(
                    top, text="Enter New Quantity", padx=5, pady=5, font=('Helvetica', 15, 'bold'))
                quantityLabel.grid(row=1, column=0)

                quantityEntry = Entry(top, width=10,  font=(
                    'Comic Sans MS', 15, 'bold'))
                ...
                editbtn = Button(
                    top, text="Change", command=applyEdits, font=('Georgia', 10, 'bold'))
        ...
        def applyDiscountProcess():
            ...
            def applyDiscounts(event=''):
                ...
                try:
                    ...
                    if (discount_scheme == 'Product Total'):
                        ...
                        viewTree.set(iidEdit, column='Total',
                    ...
                    top.destroy()
                    messagebox.showinfo(
                        "Transaction Complete", "Discount Applied")
            ...
            if (iidEdit == ""):
                ...
                discountSchemeLabel = Label(
                    top, text="Discount Scheme", font=('Helvetica', 15, 'bold'))
                discountSchemeLabel.grid(row=0, column=0, padx=5, pady=10,)

                schemeType = ttk.Combobox(top, width=15, values=[
                                          'Sales Price', 'Product Total'], font=('Comic Sans MS', 15, 'bold'))
                schemeType.grid(row=0, column=1, padx=5, pady=10,)
                schemeType.current(0)

                quantityLabel = Label(
                    top, text="Enter New Value",  font=('Helvetica', 15, 'bold'))
                quantityLabel.grid(row=2, column=0, padx=5, pady=10,)

                discountedValue = Entry(
                    top, width=15,  font=('Comic Sans MS', 15, 'bold'))
        ...
        self.searchlabel = Label(self.billingSearchFrame, text=" Name", font=(
            'Helvetica', 12, 'bold'), bg='white')
        self.searchlabel.grid(column=0, row=1, padx=15)
        # for search bar
        self.billingSearchEntry = Entry(self.billingSearchFrame, width=35, font=(
            'Helvetica', 20, 'bold'), bg='#f7eeee')
        ...
        treeStyle.configure("mystyle.Treeview", highlightthickness=1,
                            bd=0, rowheight=25, font=('Georgia', 13))
        # treeStyle.layout('mystyle.Treeview',[('mystyle.Treeview.treearea',{'sticky':'nswe'})])

       # treeview
        viewTree = ttk.Treeview(
            self.billingFrame, height=10, style="mystyle.Treeview")
        # Define Columns
        ...
        # Delete buttons
        self.additem = Button(self.billingButtonFrame, text="Delete", cursor='X_cursor', font=(
            'Comic Sans MS', 12), bg="#f54949", width=10, command=removeSelectedRow)
        self.additem.grid(column=0, row=3, sticky="n",
                          padx=10, pady=10, ipadx=8)
        ...
        saveBillButton.grid(column=0, row=4, sticky="n",
                            padx=10, pady=10, ipadx=8)
        ...
        clear_Billing.grid(column=0, row=5, sticky="n",
                           padx=10, pady=20, ipadx=8)
    ...
    def createRecord(self):
        ...
        try:
            ...
            messagebox.showwarning("Insufficient Record",
                                   "All Fields Required")
    ...
    def navigationFrame_view(self, tab):
        ...
        self.labelsearchby = Label(
            searchhelpFrame, text="Search By", font=('Comic Sans MS', 15, 'bold'))
        self.labelsearchby.grid(padx=5)
        helpLabel = Label(self.searchLabelFrame,
                          text='(Set Search filter)', font=('Comic Snas MS', 12))
        ...
        def printBill():
            try:
                ...
                messagebox.showerror(
                    'Invalid Request', 'Bill Selection Required')
            except KeyError:
        ...
        def displayBillSearch(event=''):
            ...
            try:
                ...
                if (key == ""):
                    ...
                    result = collection.find(
                        {key: {'$regex': searchValue, '$options': searchFilter}})
                    connection.close()
                    for x in result:
                        example.append(
                            x['Customer Name'] + ' ------  ' + x['Date'] + '------' + x['Contact Number'])
        ...
        # for search bar
        billSearchEntry = Entry(self.searchLabelFrame,
                                width=38, font=('Comic Snas MS', 20))
        ...
        # for listbox
        itemlistbox_view = Listbox(
            listboxFrame, width=60, height=4, bg="#e8eddf", font=('Comic Snas MS', 15))
        ...
        lbl = Label(informationFrame, text="Bill Details", font=(
            'Comic Snas MS ', 15, 'bold', 'underline'))
        lbl.grid(row=0, column=0, columnspan=4)
        # date label
        Viewdatelabel = Label(
            informationFrame, text='Date : ', font=('Helvetica', 12))
        Viewdatelabel.grid(row=1, column=0)

        dateLabel = Label(informationFrame, text='--/--/----',
        ...
        timeLabel = Label(informationFrame, text='--:-- --',
                          font=('Comic Snas MS', 12, 'bold'))
        timeLabel.grid(row=2, column=1, padx=10)

        helloatLabel = Label(
            informationFrame, text='Contact Number', font=('Hevetica', 12))
        helloatLabel.grid(row=1, column=4)

        helloat = Label(informationFrame, text=' ----------- ',
                        font=('Comic Sans MS', 10, 'bold'))
        helloat.grid(row=1, column=5)

        # Bill Total Labels
        billTotal = Label(informationFrame, text='Bill Total ',
                          font=('Helvetica', 15, 'bold'))
        billTotal.grid(row=2, column=2, padx=20)

        billTotalLabel = Label(informationFrame, text='------ /-',
        ...
        def customerCopy():
            ...
            for items in data['Products']:
                ...
                # print(data)
                c.drawCentredString(
                    x+360, y, str(data['Products'][items]['Sales Price']))  # x+345
                c.drawCentredString(x+445, y, qty)
                c.drawCentredString(
                    x+520, y, str(data['Products'][items]['Product Total']))
                cnt += 1
                y += 20
            ...
            try:
                ...
                messagebox.showerror(
                    "Permission Denied", "The bill is currently open. Please close the file to display new bill.")

        # Button Frame GUI

        dspbill = Button(self.buttonFrame, text="Display Bill",
                         command=printBill, font=('Helvetica', 15, 'bold'))
        dspbill.pack(side="top", pady=10)

        prntBill = Button(self.buttonFrame, text="Print Bill",
                          command=customerCopy, width=15, font=('Helvetica', 15, 'bold'))
    ...
    def navigationFrame_settings(self, tab):
        ...
        def displayContents():
            try:
                ...
                connection.close()
                receiverLabel = Label(
                    self.displayFrame, text=" Send mail using :", font=('Helvetica', 10, 'bold'))
                receiverLabel.grid(row=0, column=0, padx=10,
                                   pady=10, sticky='w')
                ...
                self.sender_emaildata = Label(
                    self.displayFrame, text=sysData['sender_email'], font=('Helvetica', 10, 'bold'))
                self.sender_emaildata.grid(row=0, column=1, padx=10, pady=10)

                senderPasswordLabel = Label(
                    self.displayFrame, text=" Sender email password :", font=('Helvetica', 10, 'bold'))
                senderPasswordLabel.grid(
                    row=2, column=0, padx=10, pady=10, sticky='w')

                infoLabel = Label(
                    self.displayFrame, text=" (Password of the sender's email address)")
                infoLabel.grid(row=3, column=0, columnspan=2,
                               padx=10, pady=0, sticky='w')

                self.sender_passworddata = Label(self.displayFrame, text='*********', font=(
                    'Helvetica', 10, 'bold'))  # sysData['sender_password']
                self.sender_passworddata.grid(
                    row=2, column=1, padx=10, pady=10)

                receiverLabel = Label(
                    self.displayFrame, text=" Receive mail in: ", font=('Helvetica', 10, 'bold'))
                receiverLabel.grid(row=4, column=0, padx=10,
                                   pady=10, sticky='w')
                ...
                masterPasswordLabel = Label(
                    self.displayFrame, text="Master Password :", font=('Helvetica', 10, 'bold'))
                ...
                self.master_passworddata = Label(
                    self.displayFrame, text=sysData['master_password'], font=('Helvetica', 10, 'bold'))
                self.master_passworddata.grid(
                    row=6, column=1, padx=10, pady=10)

                upd_btn = ttk.Style()
                upd_btn.configure('Update.TButton', width=10,
    ...
    def navigationFrame_customerDetails(self, tab):
        ...
        def customerHistory():
            ...
            lbl_name = Label(self.customerDisplay_top, text="Name",
                             bg='white', font=('Hevetica', 17, 'bold'))
            lbl_name.grid(row=0, column=0, pady=20)

            ent_name = Entry(self.customerDisplay_top,
                             font=("Helvetica", 15, 'bold'))
            ent_name.grid(row=0, column=1)

            lbl_phone = Label(self.customerDisplay_top, text="Contact Number",
                              bg='white', font=('Hevetica', 17, 'bold'))
            ...
            btn_search = Button(self.customerDisplay_top, text='Search', command=searchCustomer,
                                bg='#3399ff', fg='#ffffff', border=0, font=('Comic San MS', 17, 'bold'))
            ...
            lbl_BillDetails = Label(self.customerDisplay_dwn, text='Bill Records', font=(
                'Comic Sans MS', 10, 'bold'))
            lbl_BillDetails.grid(row=0, column=0, columnspan=3)

            billList = Listbox(self.customerDisplay_dwn, bg='#ffffff',
                               selectmode='Single', heigh=20, width=50, font=('Helvetica', 12, 'bold'))
            billList.grid(row=1, column=0, pady=20)

            viewScrollbar = Scrollbar(
                self.customerDisplay_dwn, orient=VERTICAL)
            viewScrollbar.config(command=billList.yview)
            viewScrollbar.grid(row=1, ipadx=5, column=5, sticky='ns')

            btn_dsp = Button(self.customerDisplay_dwn,
                             text='Display', font=('Helvetica', 15, 'bold'))
        ...
        ViewTime = Label(self.customerBillDisplay, bg='#ffffff',
                         text='Time:', font=('Hevetica', 12))
        ViewTime.grid(row=2, column=0)
...
class AuthUser(Tk):
    def __init__(self):
        ...
        label1 = Label(detailsframe, image=test,
                       width=200, height=200, bg='#ffffff')
        label1.image = test
        label1.pack()
        ...
        test = ImageTk.PhotoImage(image)
        locklbl = Label(passwordFrame, image=test,
                        height=50, width=50, bg='#ffffff')
        locklbl.image = test
        locklbl.grid(row=0, column=0)

        self.password_entry = Entry(
            passwordFrame, border=0, width=15, font=('default', 12,), bg='#f0f3f7')
        ...
        loginbutton = Button(detailsframe, text='Get Access', border=0, width=15, bg='#151FC4',
                             fg='#ffffff', font=('Helvetica', 13, 'bold'), command=self.checkPass)
