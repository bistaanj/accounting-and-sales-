...
class Window(Tk):
    ...
    def navigationFrame_inventory(self,tab):
        ...
        s_btn = ttk.Style()
        s_btn.configure('TButton', height=3, width = WR*20,border=0,
         background=buttonBg,
    ...
    def addNewRecord(self):
        ...
        productNameEntry = Entry(self.displayLabel, width = WR*50, border=0, bg='#CED7D7', font=('Helvetica',15,'bold'))
        ...
        quantityEntry = Entry(self.displayLabel, width = WR*10, font=('Helvetica',15,'bold'),
                              border=0, bg='#CED7D7')
        ...
        productCostEntry = Entry(self.displayLabel, width = WR*20, border=0, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        locationEntry = Entry(self.displayLabel, width = WR*20,border=0, bg='#CED7D7', font=('Helvetica', 15, 'bold'))
        ...
        productDescription = Entry(self.displayLabel, border = 0,width = WR*50,bg='#CED7D7', font=('Helvetica',15,'bold'))
    ...
    def updateInventory(self):
        ...
        def displayUpdatePopup(displayText,cmd):
            ...
            self.updateEntry = Entry(self.UpdatePopUp, width = WR*20)
        ...
        self.searchEntry = Entry(self.searchFrame, width = WR*40, bg='#4F83FC', fg='#FFFFFF',border=0, font=('Comic Sans MS', 15))
        ...
        viewTree.column('#0', width = WR*60, minwidth = WR*10, anchor=CENTER)
        viewTree.column('Product Name', width = WR*380, anchor=W)
        viewTree.column('Cost Price', width = WR*138, anchor=CENTER)
        viewTree.column('Sales Price', width = WR*138, anchor=CENTER)
        viewTree.column('Quantity', width = WR*130, anchor=CENTER)
        viewTree.column('Units', width = WR*110, anchor=CENTER)
        viewTree.column('Location', width = WR*150, anchor=CENTER)
    ...
    def viewInventory(self):
        ...
        self.searchEntry = Entry(searchFrame, width = WR*40, bg='#4F83FC', fg='#FFFFFF',border=0, font=('Comic Sans MS', 20))
        self.searchEntry.grid(column=1, row=1, padx=10, pady=10, sticky="w")
        self.searchEntry.insert(0,'search for...')
        self.searchEntry.bind('<FocusIn>', clearPlaceHolder)
        self.searchEntry.bind('<KeyRelease>', displaySearchResult)
        ...
        viewTree.column('#0', width = WR*60, minwidth = WR*10, anchor=CENTER)
        viewTree.column('Product Name', width = WR*200, anchor=W)
        viewTree.column('Cost Price', width = WR*138, anchor=CENTER)
        viewTree.column('Sales Price', width = WR*138, anchor=CENTER)
        viewTree.column('Quantity', width = WR*130, anchor=CENTER)
        viewTree.column('Units', width = WR*110, anchor=CENTER)
        viewTree.column('Location', width = WR*150, anchor=CENTER)
        viewTree.column('Purchased From', width = WR*150, anchor=CENTER)
    ...
    def navigationFrame_billing(self,tab):
        ...
        def completeBilling():
            ...
            if (len(self.productsInBill)<1):
                ...
                if(proceedBilling == 1):
                    ...
                    askEntry = Entry(top, width = WR*30, font=('Comic Sans MS', 15, 'bold'))
                    ...
                    phnNumEntry = Entry(top, width = WR*30, font=('Comic Sans MS', 15, 'bold'))
                    phnNumEntry.grid(row = 1, column = 1, padx = 5, pady = 5)
                    phnNumEntry.bind('<KeyRelease>',validateContact)
                    btn = Button(top, text ="Enter", width = WR* 10, command = saveBilltoDbs)
        ...
        def billingProcess():
            try:
                ...
                askQuantityEntry = Entry(top, width = WR*10, font=('Comic Sans MS', 15, 'bold'))
                askQuantityEntry.grid(row=2, column=1)
                askQuantityEntry.focus()
                askQuantityEntry.bind('<Return>', displayToBillView)

                okBtn = Button(top, text="Sell", padx=5,pady=10, width = WR* 8,font=('Georgia', 10,'bold'), command=displayToBillView)
        ...
        def billingEditProcess():
            ...
            if (iidEdit == ""):
                ...
                quantityEntry = Entry(top, width = WR*10,  font=('Comic Sans MS', 15, 'bold'))
        ...
        def applyDiscountProcess():
            ...
            if (iidEdit==""):
                ...
                schemeType =ttk.Combobox(top, width = WR* 15, values=['Sales Price','Product Total'],font=('Comic Sans MS', 15, 'bold'))
                ...
                discountedValue = Entry(top, width = WR*15,  font=('Comic Sans MS', 15, 'bold'))
        ...
        #for search bar
        self.billingSearchEntry = Entry(self.billingSearchFrame,width = WR*35,font=('Helvetica', 20,'bold'), bg='#f7eeee')
        ...
        self.searchButton = Button(self.billingSearchFrame, text="Add Product", font=(
            'Helvetica', 14, 'bold'), width = WR*10, bg="#6aeb7b", command=billingProcess)
        self.searchButton.grid(column = 5, row = 1)

        #for listbox
        self.itemlistbox = Listbox(
            self.billingSearchFrame, width = WR*80, height=5, bg="#e8eddf")
       ...
       #treeview 
        ...
        viewTree.column('#0', width = WR* 40, minwidth = WR* 20, anchor = CENTER)
        viewTree.column('Product Name', width = WR*300, anchor = 'w')
        viewTree.column('Quantity', width = WR* 130, anchor = CENTER)
        viewTree.column('Units', width = WR* 60, anchor = CENTER)
        viewTree.column('Sales Price', width = WR*130, anchor=CENTER)
        viewTree.column('Total', width = WR*130, anchor=CENTER)
        ...
        self.editbutton = Button(
            self.billingButtonFrame, text="Edit", bg="#91cf92", command=billingEditProcess, width = WR*10, font=('Comic Sans MS',12))
        self.editbutton.grid(column=0,row=2,ipadx=8,padx=10, pady = 10)


        #Delete buttons
        self.additem = Button(self.billingButtonFrame, text="Delete",cursor ='X_cursor',font=('Comic Sans MS',12),bg="#f54949", width = WR*10, command=removeSelectedRow)
        ...
        saveBillButton = Button(self.billingButtonFrame, text="Save Bill",
                                     width = WR*10,  height=2, command=completeBilling,
        ...
        clearBilling = Button(self.billingButtonFrame, text="Clear Billing", bg="#f54949",cursor ='X_cursor',
                              width = WR*10,  font=('Helvetica', 12, 'bold'), command=clearBilling)
        ...
        applyDiscountToProduct = Button(self.billingButtonFrame, text="Apply Discounts", bg='#648EF1', fg='#FFFFFF', cursor='hand2',
                                        width = WR*10,  font=('Helvetica', 12, 'bold'), command =applyDiscountProcess)
        ...
        self.totalAmountLabel = Label(
            self.amountFrame, width = WR*10, text='Total :', bg='#4A2727', font=amountLabel, fg='#FAF712')
        self.totalAmountLabel.grid(row = 2, column = 0,  pady =0, sticky = 'n')

        self.billingAmountLabel = Label(
            self.amountFrame, width = WR*12, text="", bg="#4A2727", font=amountTotal, fg='#FAF712')
        self.billingAmountLabel.grid(row=2, column=1, sticky="n",  pady=0)
        self.billingAmountLabel.config(text=self.billingTotalAmount)

        #print receipt
        # self.printreceipt= Button(self.billingFrame,text="Print Receipt",bg="#7ee081",width = WR*10)
    ...
    def navigationFrame_view(self, tab):
        ...
        viewcombobox_search = ttk.Combobox(
            self.searchLabelFrame, textvariable=4, width = WR*12, font = ('Comic Sans MS', 15))
        ...
        #for search bar
        billSearchEntry = Entry(self.searchLabelFrame, width = WR*38, font =('Comic Snas MS', 20 ))
        ...
        #                         width = WR*"10", bg="#6aeb7b", command=displayBillSearch)
        # viewSearch_btn.pack(side='left')

        
        #for listbox
        itemlistbox_view = Listbox(listboxFrame, width = WR*60, height=4, bg="#e8eddf", font =('Comic Snas MS', 15 ))
        ...
        view_viewTree.column('#0', width = WR*60, minwidth = WR*25, anchor=CENTER)
        view_viewTree.column('Product Name', width = WR*425, anchor=W)
        view_viewTree.column('Sales Price', width = WR*150, anchor=CENTER)
        view_viewTree.column('Quantity', width = WR*150, anchor=CENTER)
        view_viewTree.column('Product Total', width = WR*150, anchor=CENTER)
        ...
        def customerCopy():
            ...
            # logo=('./res/logo.jpg')
            # c.drawImage(logo,10,10,height = 20,width = WR*20)
        ...
        prntBill = Button(self.buttonFrame, text = "Print Bill", command=customerCopy, width = WR* 15,font = ('Helvetica', 15,'bold'))
    ...
    def navigationFrame_settings(self,tab):
        ...
        def displayTop(displayText,cmd):
            ...
            self.AppsSettingsEntry = Entry(self.UpdatePopUp, width = WR*20)
        ...
        def displayContents():
            try:
                ...
                upd_btn=ttk.Style()
                upd_btn.configure('Update.TButton',width = WR* 10, font=('Times New Roman',10))
