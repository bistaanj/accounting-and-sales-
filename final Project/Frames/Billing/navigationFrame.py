...
def navigationFrame(self, tab):
    ...
    self.billtypelabel = Label(self.billingtypeFrame, text="VAT BILLING",
                               bg='#FFFFFF', font=(self.fontToUse, int(FR*30), 'bold', 'underline'), fg="#5A63F5", border=0)
    self.billtypelabel.pack(fill="both", side="top")

    self.searchlabel = Label(self.billingSearchFrame, text=" Name", font=(
        self.fontToUse, int(FR*12), 'bold'), bg='white')
    ...
    # add button button
    self.searchButton = Button(self.billingSearchFrame, text="Add Product", font=(self.fontToUse, int(
    ...
    self.editbutton = Button(self.billingButtonFrame, text="Edit", bg="#91cf92", command=lambda: billingProcess.billingEditProcess(
        self, viewTree), width=int(WR*10), font=(self.fontToUse, int(FR*12)))
    self.editbutton.grid(column=0, row=2, ipadx=8, padx=10, pady=10)

    # Delete buttons
    self.additem = Button(self.billingButtonFrame, text="Delete", cursor='X_cursor', font=(self.fontToUse, int(
    ...
    clear_Billing = Button(self.billingButtonFrame, text="Clear Billing", bg="#f54949", cursor='X_cursor',
                           width=int(WR*10),  font=(self.fontToUse, int(FR*12), 'bold'), command=lambda: billingOptions.clearBilling(self, viewTree))
    ...
    applyDiscountToProduct = Button(self.billingButtonFrame, text="Apply Discounts", bg='#648EF1', fg='#FFFFFF', cursor='hand2',
                                    width=int(WR*10),  font=(self.fontToUse, int(FR*12), 'bold'), command=lambda: billingOptions.applyDiscountProcess(self, viewTree))
    applyDiscountToProduct.grid(
        column=0, row=6, sticky="n", padx=10, pady=20, ipadx=8)

    # for vatable amount
    self.VatableAmountLabel = Label(
        self.amountFrame, width=int(WR*10), text='Vatable        :', bg='#4A2727', font=(self.fontToUse, int(FR*22), 'bold'), fg='#FAF712')
    self.VatableAmountLabel.grid(row=1, column=0,  pady=0, sticky='n')

    self.billingVatableAmountLabel = Label(
        self.amountFrame, width=int(WR*12), text="", bg="#4A2727", font=(self.fontToUse, int(FR*22), 'bold'), fg='#FAF712')
    ...
    self.totalAmountLabel = Label(
        self.amountFrame, width=int(WR*10), text='Grand Total :', font=(self.fontToUse, int(FR*22), 'bold'), bg='#4A2727', fg='#FAF712')
