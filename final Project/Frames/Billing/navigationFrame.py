...
def navigationFrame(self,tab):
    ...
    self.btn_addProduct = ttk.Button(self.buttonFrame, text = "VAT Billing",style ='TButton', command =lambda:vat.vat_billing(self,viewTree))
    self.btn_addProduct.grid(column = 0 , row = 1, pady = 10)

    self.btn_update = ttk.Button(self.buttonFrame, text="Order", style='TButton', command=lambda:order.order(self,viewTree))
    ...
    #add button button
    self.searchButton = Button(self.billingSearchFrame, text="Add Product", font = ('Helvetica', int(FR*14), 'bold'), width = int(WR*10), bg="#6aeb7b", command=lambda:billingProcess.billingProcess(self,viewTree))
    ...
    #Edit Button
    self.editbutton = Button(self.billingButtonFrame, text="Edit", bg="#91cf92", command=lambda:billingProcess.billingEditProcess(self,viewTree), width = int(WR*10), font=('Comic Sans MS',int(FR*12)))
    self.editbutton.grid(column=0,row=2,ipadx=8,padx=10, pady = 10)


    #Delete buttons
    self.additem = Button(self.billingButtonFrame, text="Delete",cursor ='X_cursor',font=('Comic Sans MS',int(FR*12)),bg="#f54949", width = int(WR*10), command=lambda:billingOptions.removeSelectedRow(self,viewTree))
