...
def navigationFrame(self, tab):
    ...
    def customerHistory():
        ...
        def displayBill():
            ...
            if self.currentDateType == "BS":
                addDate = datetime.date(
                    datetime.strptime(bill['Date'], "%d/%m/%Y"))
                bsdate = convert_AD_to_BS(
                    addDate.year, addDate.month, addDate.day)
                dateLabel.config(
                    text=str(bsdate[0])+"-"+str(bsdate[1])+"-"+str(bsdate[1]))
            else:
        ...
        lbl_name = Label(self.customerDisplay_top, text="Name",
                         bg='white', font=(self.fontToUse, int(FR*14), 'bold'))
        lbl_name.grid(row=0, column=0, pady=20)

        ent_name = Entry(self.customerDisplay_top, font=(
        ...
        billList = Listbox(self.customerDisplay_dwn, bg='#ffffff', selectmode='Single',
                           heigh=14, width=int(WR*38), font=(self.fontToUse, int(FR*12), 'bold'))
        ...
        btn_dsp = Button(self.customerDisplay_dwn, text='Display',
                         command=displayBill, font=(self.fontToUse, int(FR*15), 'bold'))
    ...
    s_btn.configure('TButton', height=int(HR*3), width=int(WR*20), border=0,
                    background=buttonBg,
                    font=(self.fontToUse, int(FR*14), 'bold'))
    s_btn.map('TButton',
    ...
    lbl = Label(self.customerBillDisplay, text="Bill Details",
                bg='#ffffff', font=(self.fontToUse, 15, 'bold', 'underline'))
    lbl.grid(row=0, column=0, columnspan=4)
    # date label
    Viewdatelabel = Label(
        self.customerBillDisplay, bg='#ffffff', text='Date : ', font=(self.fontToUse, int(FR*12)))
    ...
    Viewcustumername = Label(
        self.customerBillDisplay, bg='#ffffff', text='Customer Name: ', font=(self.fontToUse, int(FR*12)))
    Viewcustumername.grid(row=3, column=0)

    customerNameLabel = Label(
        self.customerBillDisplay, bg='#ffffff', text='---------', font=(self.fontToUse, int(FR*12), 'bold'))
    ...
    helloatLabel = Label(
        self.customerBillDisplay, bg='#ffffff', text='Contact Number', font=(self.fontToUse, int(FR*12)))
