...
def navigationFrame(self, tab):
    ...
    self.labelsearchby = Label(self.searchhelpFrame, text="Search By", font=(
        self.fontToUse, int(FR*15), 'bold'))
    self.labelsearchby.grid(padx=5)
    self.helpLabel = Label(self.searchLabelFrame, text='(Set Search filter)', font=(
        self.fontToUse, int(FR*12)))
    ...
    def displayBillSearch(event=''):
        ...
        try:
            ...
            if (key == ""):
                ...
                # connection.close()
                for x in result:
                    example.append(x['Customer Name'] + ' ------  ' +
                                   x['Date'] + '------' + x['Contact Number'])
    ...
    self.viewcombobox_search = ttk.Combobox(self.searchLabelFrame, textvariable=4, width=int(
        WR*12), font=(self.fontToUse, int(FR*15)), state='readonly')
    ...
    # for search bar
    self.billSearchEntry = Entry(self.searchLabelFrame, width=int(
        WR*38), font=(self.fontToUse, int(FR*20)))
    ...
    # for listbox
    self.itemlistbox_view = Listbox(self.listboxFrame, width=int(
        WR*60), height=int(HR*4), bg="#e8eddf", font=(self.fontToUse, int(FR*15)))
    ...
    self.lbl = Label(self.informationFrame, text="Bill Details", font=(
        self.fontToUse, int(FR*15), 'bold', 'underline'))
    self.lbl.grid(row=0, column=0, columnspan=4)
    # date label
    self.Viewdatelabel = Label(
        self.informationFrame, text='Date : ', font=(self.fontToUse, int(FR*12)))
    self.Viewdatelabel.grid(row=1, column=0)

    self.dateLabel = Label(self.informationFrame, text='--/--/----',
                           font=(self.fontToUse, int(FR*15), 'bold'))
    self.dateLabel.grid(row=1, column=1)

    # custumer name label
    self.Viewcustumername = Label(
        self.informationFrame, text='Customer Name: ', font=(self.fontToUse, int(FR*12)))
    self.Viewcustumername.grid(row=1, column=2)

    self.customerNameLabel = Label(
        self.informationFrame, text='---------', font=(self.fontToUse, int(FR*12), 'bold'))
    self.customerNameLabel.grid(row=1, column=3)

    self.ViewTime = Label(self.informationFrame,
                          text='Time:', font=(self.fontToUse, int(FR*12)))
    self.ViewTime.grid(row=2, column=0)

    self.timeLabel = Label(self.informationFrame, text='--:-- --',
                           font=(self.fontToUse, int(FR*12), 'bold'))
    self.timeLabel.grid(row=2, column=1, padx=10)

    self.helloatLabel = Label(
        self.informationFrame, text='Contact Number', font=(self.fontToUse, int(FR*12)))
    self.helloatLabel.grid(row=1, column=4)

    self.helloat = Label(self.informationFrame, text=' ----------- ',
                         font=(self.fontToUse, int(FR*10), 'bold'))
    self.helloat.grid(row=1, column=5)

    # Bill Total Labels
    self.billTotal = Label(self.informationFrame, text='Bill Total ', font=(
        self.fontToUse, int(FR*15), 'bold'))
    self.billTotal.grid(row=2, column=2, padx=20)

    self.billTotalLabel = Label(self.informationFrame, text='------ /-',
                                bg='#F2F81D', fg='#164ECF', font=(self.fontToUse, int(FR*15), 'bold'))
    ...
    view_viewTree = ttk.Treeview(
        self.billFrame, height=int(HR*10), style="mystyle.Treeview")
    ...
    self.dspbill = Button(self.buttonFrame, text="Display Bill",
                          command=printBill, font=(self.fontToUse, int(FR*15), 'bold'))
    self.dspbill.pack(side="top", pady=10)

    self.prntBill = Button(self.buttonFrame, text="Print Bill", command=customerCopy, width=int(
