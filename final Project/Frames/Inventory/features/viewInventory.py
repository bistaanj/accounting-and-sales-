...
def viewInventory(self, tab):
    ...
    totalSoldLabel = Label(self.displayFrame, text="",
                           font=(self.fontToUse, int(FR*15)))
    totslOrderLabel = Label(self.displayFrame, text="",
                            font=(self.fontToUse, int(FR*15)))

    self.searchEntry = Entry(searchFrame, width=int(
        WR*40), bg='#4F83FC', fg='#FFFFFF', border=0, font=(self.fontToUse, int(FR*20)))
    ...
    def showDetailsOfProduct(event=''):
        ...
        if(row_iid != ''):
            ...
            totalSoldLabel.config(text='Items sold: ' +
                                  str(databaseRow['Sold']))
            totslOrderLabel.config(
                text='Items Ordered: ' + str(databaseRow['Order']))
        else:
            viewTree.pack_configure(expand=1)
            totslOrderLabel.pack_forget()
            totalSoldLabel.pack_forget()
    ...
    for x in inventorydata:
        ...
        if (qnty == 0):
            qnty = "Out of Stock"
        viewTree.insert(parent='', index=END, iid=(x["_id"]), text=(self.txt+1), values=(
            x['Product Name'], x['Cost Price'], x['Sales Price'], qnty, x['Units'], x['Purchased From']))
        self.txt += 1
