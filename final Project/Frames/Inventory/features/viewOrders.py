...
def viewOrders(self):
                ...
                for i in allOrders:
                    for a in i['Products']:
                        if a.lower() in item.lower():
                            process1 = {
                                'date': i['Date'], 'cName': i['Customer Name'], 'product': i['Products'][a]}
        ...
        try:
            if (key == ""):
                ...
                if(key == 'By Product'):
                    ...
                    viewTree.heading(
                        'Customer Name', text='Customer Name', anchor=CENTER)
                    ...
                    for x in allOrders:
                        if len(contact) > 0:
                            if x['Contact Number'] == contact:
                                self.nameAndPhoneNumber[x['_id']] = x['Customer Name'] + \
                                    ' --- ' + x['Contact Number']
                                raw = {}
                                raw = x['Products']
                                for k in list(raw):
                    ...
                    viewTree.heading(
                        'Customer Name', text='Product Name', anchor=CENTER)

        except ValueError:
            messagebox.showerror("Invalid Request", "Set Search Filter")

    PtypeCombo = ttk.Combobox(topFrame, background='#CED7D7', width=int(
        WR*10), values=['By Customer', 'By Product'], font=(self.fontToUse, int(FR*10), 'bold'), state='readonly')
    ...
    productNameLabel = Label(
        topFrame, text="Product Name:", font=(self.fontToUse, int(FR*12)))
    productNameLabel.grid_forget()
    searchBox = Entry(topFrame, font=(
        self.fontToUse, int(FR*13), 'bold'), width=int(WR*20))
    searchBox.grid_forget()

    customerNameLabel = Label(
        topFrame, text="Customer Name: ", font=(self.fontToUse, int(FR*12)))
    customerNameLabel.grid(column=1, row=1, pady=5, padx=15)

    customerNameEntry = Entry(topFrame, font=(
        self.fontToUse, int(FR*11), 'bold'), width=int(WR*20))
    customerNameEntry.grid(column=2, row=1, pady=5, padx=15)

    customerPhoneLabel = Label(
        topFrame, text="Mobile Number: ", font=(self.fontToUse, int(FR*12)))
    customerPhoneLabel.grid(column=1, row=2, pady=5, padx=15)

    customerPhoneEntry = Entry(topFrame, font=(
        self.fontToUse, int(FR*11), 'bold'), width=int(WR*20))
    customerPhoneEntry.grid(column=2, row=2, pady=5, padx=15)
    customerPhoneEntry.bind('<KeyRelease>', validateContact)

    btn_search = Button(topFrame, text='Search', bg='#3399ff', fg='#ffffff', border=0, font=(
        self.fontToUse, int(FR*13), 'bold'), command=displayOrderSearch)
    btn_search.grid(row=1, column=3, padx=25, pady=10, rowspan=2)

    customerName = Label(orderDetailsFrame, text='',
                         font=(self.fontToUse, int(FR*13)))
    customerName.pack()

    itemlistbox = Listbox(orderDetailsFrame, width=int(
        WR*20), height=int(HR*3), bg="#e8eddf", font=(self.fontToUSe, int(FR*14)))
    ...
    viewTree = ttk.Treeview(
        orderDetailsFrame,  style="mystyle.Treeview", height=13)
    ...
    checkoutSelectedButton = Button(bottomFrame, text="Checkout Selected",
                                    bg='#3399ff', fg='#ffffff', border=0, font=(self.fontToUse, int(FR*13)))
    checkoutSelectedButton.pack(side=LEFT, padx=20)

    checkoutAllProductsButton = Button(bottomFrame, text="Checkout All Product",
                                       bg='#3399ff', fg='#ffffff', border=0, font=(self.fontToUse, int(FR*13)))
    checkoutAllProductsButton.pack_forget()

    checkoutAllOrderButton = Button(bottomFrame, text="Checkout All Order",
                                    bg='#3399ff', fg='#ffffff', border=0, font=(self.fontToUse, int(FR*13)))
    checkoutAllOrderButton.pack_forget()
