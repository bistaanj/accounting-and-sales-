...
def checkoutOrders(self):
    ...
    def getallOrders():
        ...
        for x in allProducts:
            productDetails[x['Product Name']] = {
                'orderQuantity': x['Order'], 'Sales Price': x['Sales Price']}
    ...
    def editOrder():
        ...
        def validateQuantity(e):
            try:
                if int(askOrderQuantityEntry.get()) > productDetails[product]['orderQuantity']:
                    ...
                    askOrderQuantityEntry.insert(
                        0, int(productDetails[product]['orderQuantity']))
            except ValueError:
                askOrderQuantityEntry.delete(-1, 'end')
        ...
        if iidEdit == "":
            ...
            availableOrderQuantity = Label(top, text="Ordered Quantity", padx=5, pady=5, font=(
                self.fontToUse, int(FR*15), 'bold'))
            availableOrderQuantity.grid(row=0, column=0)

            askOrderQuantityEntry = Entry(top, width=int(
                WR*10), font=(self.fontToUse, int(FR*15), 'bold'))
            askOrderQuantityEntry.insert(
                0, productDetails[product]['orderQuantity'])
            ...
            salesPriceLbl = Label(top, text="Sales Price (RS)", padx=5, pady=5, font=(
                self.fontToUse, int(FR*15), 'bold'))
            salesPriceLbl.grid(row=1, column=0)

            askSalesPriceEntry = Entry(top, width=int(
                WR*10), font=(self.fontToUse, int(FR*15), 'bold'))
            askSalesPriceEntry.insert(
                0, productDetails[product]['Sales Price'])
            ...
            okBtn = Button(top, text="Sell", padx=5, pady=10, width=int(WR*8), font=(self.fontToUse, int(FR*10), 'bold'),
                           command=changeOrder)
    ...
    def modifyTotalAmount():
        ...
        billingAmountLabel.config(
            text=int(billingTotalAmount+0.13*billingTotalAmount))
        self.executing = False
    ...
    def addProduct(event='', product=""):
        ...
        def displayToBill(event="", product=""):
            try:
                ...
                if product != "":
                    orderQuatity = int(
                        productDetails[product]['orderQuantity'])
        ...
        def saveBilltoDbs(event=''):
            try:
                ...
                for product in productsInBill:
                    ...
                    newValue = orgValue - \
                        int(productsInBill[product]['Quantity'])
                ...
                billDict['Grand Total'] = int(
                    int(billingTotalAmount)+0.13*int(billingTotalAmount))
                ...
                messagebox.showerror("Insuccifient Data",
    ...
    billtypelabel = Label(billingtypeFrame, text="Checkout Orders",
                          bg='#e8eddf', font=(self.fontToUse, int(FR*30), 'bold', 'underline'), fg="#5A63F5", border=0)
    billtypelabel.pack(fill="both", side="top")

    searchlabel = Label(billingSearchFrame, text="Product Name", font=(
        self.fontToUse, int(FR*12), 'bold'), bg='#e8eddf')
    searchlabel.grid(column=0, row=1, padx=15)
    # for search bar
    billingSearchEntry = Entry(billingSearchFrame, width=int(
        WR*35), font=(self.fontToUse, int(FR*20), 'bold'), bg='#f7eeee')
    billingSearchEntry.grid(column=1, row=1, padx=15)
    billingSearchEntry.bind('<KeyRelease>', displayProductOptions)

    # add button button
    addAllProduct = Button(billingSearchFrame, text="Add All Product", font=(
        self.fontToUse, int(FR*12), 'bold'), width=int(WR*12), bg="#6aeb7b", command=addAllProduct)
    ...
    vtStyle.configure('Treeview.Heading', font=(
        self.fontToUse, int(FR*12), 'bold'))
    treeStyle = ttk.Style()

    treeStyle.configure("mystyle.Treeview", highlightthickness=1, bd=0, rowheight=int(
        HR*25), font=(self.fontToUse, int(FR*13)))
    # treeStyle.layout('mystyle.Treeview',[('mystyle.Treeview.treearea',{'sticky':'nswe'})])

    # treeview
    viewTree = ttk.Treeview(billingFrame, height=int(
    ...
    # Edit Button
    editbutton = Button(billingButtonFrame, text="Edit", bg="#91cf92", width=int(
        WR*10), font=(self.fontToUse, int(FR*12)), command=editOrder)
    editbutton.grid(column=0, row=2, ipadx=8, padx=10, pady=10)

    # Delete buttons
    deleteItem = Button(billingButtonFrame, text="Delete", cursor='X_cursor', font=(
        self.fontToUse, int(FR*12)), bg="#f54949", width=int(WR*10), command=deleteOrder)
    ...
    saveBillButton = Button(billingButtonFrame, text="Save Bill", command=completeBilling,
                            ...
                            font=(self.fontToUse, int(FR*15)), bg='#648EF1', fg='#FFFFFF', border=0, cursor='hand2')
    saveBillButton.grid(column=0, row=4, sticky="n", padx=10, pady=10, ipadx=8)

    #amountLabel = font.Font(family = self.fontToUse, size = int(FR*22), weight = 'bold')
    #amountTotal = font.Font(family=self.fontToUse, size=int(FR*22), weight='bold')

    clear_Billing = Button(billingButtonFrame, text="Clear Billing", bg="#f54949", cursor='X_cursor',
                           width=int(WR*10),  font=(self.fontToUse, int(FR*12), 'bold'), command=clearBilling)
    ...
    applyDiscountToProduct = Button(billingButtonFrame, text="Apply Discounts", bg='#648EF1', fg='#FFFFFF', cursor='hand2',
                                    width=int(WR*10),  font=(self.fontToUse, int(FR*12), 'bold'))
    applyDiscountToProduct.grid(
        column=0, row=6, sticky="n", padx=10, pady=20, ipadx=8)

    # for vatable amount
    VatableAmountLabel = Label(
        amountFrame, width=int(WR*10), text='Vatable        :', bg='#4A2727', font=(self.fontToUse, int(FR*22), 'bold'), fg='#FAF712')
    VatableAmountLabel.grid(row=1, column=0,  pady=0, sticky='n')

    billingVatableAmountLabel = Label(
        amountFrame, width=int(WR*12), text="", bg="#4A2727", font=(self.fontToUse, int(FR*22), 'bold'), fg='#FAF712')
    ...
    totalAmountLabel = Label(
        amountFrame, width=int(WR*10), text='Grand Total :', font=(self.fontToUse, int(FR*22), 'bold'), bg='#4A2727', fg='#FAF712')
